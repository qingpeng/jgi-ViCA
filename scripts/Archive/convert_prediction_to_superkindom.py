...
parser.add_argument('-l', '--level', help ="the level to convert from, family/order/genus", required= True,default='family')
parser.add_argument('-d', '--index', help =".index file", required= True,default='all.vect.family.index')
...
level_id = {}# 0,1,2 -> real tax-id
for line in file_index_obj:
    line = line.rstrip()
    fields = line.split()
    level_id[fields[0]] = fields[1]
...
for line in file_rank_obj: # real tax-id family -> real order_id
    ...
    if level == "family":
        ...
        if "0" in tax_dict and "11" in tax_dict:
            superkindom_dict[tax_dict['11']] = tax_dict['0']
    elif level == "genus":
        if "0" in tax_dict and "20" in tax_dict:
            superkindom_dict[tax_dict['20']] = tax_dict['0']
    else:
...
for line in file_prediction_obj:# all family id from 0...]: 
    ...
    newline = "(" + superkindom_dict[level_id[str(int(float(fields[0])))]] + \
              "," + superkindom_dict[level_id[str(int(float(fields[1])))]] + ")"
