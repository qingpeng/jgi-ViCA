...
def parsemod(dir):
    ...
    file = os.path.join(dir,"sample.mod")
...
def main():
    ...
    using emmission data from MetaGeneMark')
    ...
    parser.add_argument('--mmp', help="the parameters file for metagenemark", default = "../gm_parameters/par_11.modified")
    parser.add_argument('--mod', help="the metamod file for metagenemark", default = "../MetaGeneMark_v1.mod")
    ...
    for record in records:
        ...
        ## Run metagenemark to identify coding regions
        genemarkparams = ["gmhmmp", "-m", mod, "fragment.fasta"]
        p1 = subprocess.Popen(genemarkparams, stdout=subprocess.PIPE)
        metagenemarkout, metagenemarkerr= p1.communicate()
        if p1.returncode == 0:
            probuildparams = ["--par", mmp, "--mkmod", "sample.mod", "--seq", "fragment.fasta", "--geneset", "fragment.lst", "--ORDM", "2", "--order_non", "2", "--revcomp_non", "1", "--MIN_SEQ_SIZE", minlen]
            p2 = subprocess.Popen(probuildparams, stdout=subprocess.PIPE)
            probuildout, probuilderr= p2.communicate()
            if p2.returncode == 0:
