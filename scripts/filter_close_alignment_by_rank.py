...
parser = argparse.ArgumentParser(description='A script to filter out the alignments hits close to the query in specific taxonomy rank level' )
parser.add_argument('-s', '--sequence', help ="raw sequence,virus_segment_5k.fa", required= True)
parser.add_argument('-a', '--accession', help ="prot.accession2taxid", required= True)
parser.add_argument('-i', '--input', help ="virus_segment_5k_for_MGRAST_N.fa.daa.m8", required= True)
parser.add_argument('-r', '--rank', help ="rank level family,order", required= True)
parser.add_argument('-t', '--top', help ="number of top hits to keep, 100", required= True)
parser.add_argument('-o', '--output', help ="filtered alignment output, virus_segment_5k_for_MGRAST_N.fa.daa.m8.filter",required= True) 
...
file_raw_seq_input = args.sequence

#file_raw_seq_input = "virus_segment_5k.fa"

file_accession2taxid_input = args.accession  #"prot.accession2taxid"

file_alignment_input = args.input #"virus_segment_5k_for_MGRAST_N.fa.daa.m8"

file_output_input = args.output #"virus_segment_5k_for_MGRAST_N.fa.daa.m8.filter"
...
def main(file_raw_seq, file_accession2taxid, file_alignment, file_output, rank_level,top_number):
    ...
    for line in file_alignment_obj:
        ...
        try:# make sure they belong to different family explictly,
...
main(file_raw_seq_input, file_accession2taxid_input, file_alignment_input, file_output_input,rank_level,top_number)
