...
def main():
    parser = argparse.ArgumentParser(
        description='A script to split vector file into training and testing '
                    'by different tax rank')
    parser.add_argument('-v', '--vector', help='full vector file',
                        required=True)
    parser.add_argument('-r', '--virus',
                        help='for virus classifier or not, True/False',
    ...
    for line in file_vector_obj:
        ...
        if "11" in tax_dict and "16" in tax_dict and "20" in tax_dict and "24" in tax_dict:
            ...
            if tax_dict["16"] in order_testing[tax_dict["11"]]:
                ...
                file_order_testing_obj.write(
                    str(order_id_num) + ' ' + to_svmlib(vectors_line,
                                                        feature_list) + '\n')
            else:  # if this record is for training
                if order_testing[
                    tax_dict["11"]] != []:  # only if there are >=2 sub nodes
            ...
            if tax_dict["20"] in family_testing[tax_dict["16"]]:
                ...
                file_family_testing_obj.write(
                    str(family_id_num) + ' ' + to_svmlib(vectors_line,
                                                         feature_list) + '\n')
            else:
                if family_testing[tax_dict["16"]] != []:
                    ...
                    file_family_training_obj.write(
                        str(family_id_num) + ' ' + to_svmlib(vectors_line,
                                                             feature_list) + '\n')
            ...
            if tax_dict["24"] in genus_testing[tax_dict["20"]]:
                ...
                file_genus_testing_obj.write(
                    str(genus_id_num) + ' ' + to_svmlib(vectors_line,
                                                        feature_list) + '\n')
            else:
