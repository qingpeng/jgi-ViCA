python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0 -i training.vect -o training.vect.0
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 1 -i training.vect -o training.vect.1
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 2 -i training.vect -o training.vect.2
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 3 -i training.vect -o training.vect.3
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 4 -i training.vect -o training.vect.4
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1 -i training.vect -o training.vect.0_1
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1_2 -i training.vect -o training.vect.0_1_2
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1_2_3 -i training.vect -o training.vect.0_1_2_3
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0 -i testing.vect -o testing.vect.0
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 1 -i testing.vect -o testing.vect.1
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 2 -i testing.vect -o testing.vect.2
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 3 -i testing.vect -o testing.vect.3
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 4 -i testing.vect -o testing.vect.4
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1 -i testing.vect -o testing.vect.0_1
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1_2 -i testing.vect -o testing.vect.0_1_2
python ~/GDrive/Development/Bitbucket/jgi-genelearn/scripts/pick_vectors_by_feature.py -d all_segment.fasta.vect.feature_index -f 0_1_2_3 -i testing.vect -o testing.vect.0_1_2_3
