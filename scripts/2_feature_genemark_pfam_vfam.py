...
def main():
    ...
    parser.add_argument('--genemark_path', help="genemark path, ./gmsuite",
                        required=True)
    parser.add_argument('--hmmer_path', help="HMMER path", required=True)
...
#
    ...
    hmmer_path = os.path.abspath(args.hmmer_path)
    hmmer_db = os.path.abspath(args.hmmer_db)
            ...
            # if MetaGeneMark can generate the gene prediction
                ...
                if p2.returncode == 0:  # if probuild can generate hmm model
                    ...
                    if vect_genemark:
                        ...
                        hmmscan_params_pfam = [hmmer_path+"/binaries/hmmscan",
                                               "--tblout",
                                               "fragment.fasta.aa."
                                               "hmmscan_pfam",
                                               "-E", "1e-5", hmmer_db +
                        ...
                        if p3.returncode == 0:
                            vector_pfam = parse_hmmer(
                                "fragment.fasta.aa.hmmscan_pfam")
                            line_pfam = (line_pfam +
                                         generate_line(vector_pfam))

                        hmmscan_params_vfam = [hmmer_path+"/binaries/hmmscan",
                                               "--tblout",
                                               "fragment.fasta.aa."
                                               "hmmscan_vfam", "-E",
                                               "1e-5",
                                               hmmer_db+'/vFam-B_2014.hmm',
                                               "fragment.fasta.aa"]
                        p4 = subprocess.Popen(hmmscan_params_vfam,
                                              stdout=subprocess.PIPE)
                        metamarkout, metamarkerr= p4.communicate()

                        if p4.returncode == 0:
                            vector_vfam = parse_hmmer(
                        ...
                        hmmscan_params_img = [hmmer_path+"/binaries/hmmscan",
                                              ...
                                              "hmmscan_img", "-E", "1e-5",
                                              hmmer_db+'/final_list.hmms',
                                              "fragment.fasta.aa"]
                        p5 = subprocess.Popen(hmmscan_params_img,
                                              stdout=subprocess.PIPE)
                        metamarkout, metamarkerr= p5.communicate()

                        if p5.returncode == 0:
                            vector_img = parse_hmmer(
                                "fragment.fasta.aa.hmmscan_img")
